...
\begin{frame}{A concern driven by practical modeling: ice sheets}
...
$$h_t + \Div\bq = f \qquad \to \qquad \frac{H_n - H_{n-1}}{\Delta t} + \Div \bQ_n = F_n$$
  ...
  \item the new equation is strong form ``single time-step problem''
    \begin{itemize}
...
\begin{frame}{Weak form incorporates nonnegative-thickness constraint}
...
\begin{itemize}
  ...
  \item if $H_n \in \mathcal{K}$ solves weak single time-step problem then
	  ...
	  \item information on the set where $H_n = 0$:
	    ...
	    \vspace{-6mm}
	    \begin{itemize}
	    \item[$\circ$] ``climate negative enough during time step to remove old thickness''
	    \end{itemize}
	  \end{enumerate}
\end{itemize}
\end{frame}
