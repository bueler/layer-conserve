...
$$h_t + \Div\bq = f \qquad \to \qquad \frac{H_n - H_{n-1}}{\Delta t} + \Div \bQ_n = F_n$$
  ...
  \item the new equation is strong form \alert{single time-step problem}
    ...
    \item[$\circ$] e.g.~$\theta$-methods or RK
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{itemize}
  \item assume $\bQ_n=0$ when $H_n=0$
    ...
    \end{itemize}
  \item if $H_n \in \mathcal{K}$ solves weak single time-step problem then
  
      \medskip
	  \begin{itemize}
	  \item[$\circ$] PDE applies on the set where $H_n>0$:
...
\subsection{new numerical tools which scale}
...
\begin{frame}{Example: Greenland ice sheet at 900 m res.}

\begin{columns}
\begin{column}{0.6\textwidth}
\begin{itemize}
\item question: given steady climate (= precipitation + melt) and bedrock elevations, what is shape of Greenland ice sheet?
...
\begin{center}
\includegraphics[width=0.9\textwidth,keepaspectratio=true]{grnwinset}
\end{center}
\end{column}
\end{columns}
\end{frame}
