...
\begin{center}
\alert{there is a whole class of climate modeling problems which people (scientists/modelers) are doing the wrong way}
\end{center}
  \begin{itemize}
  \item[$\circ$] especially in choice of mathematical formulation
  \end{itemize}
\visible<2>{
...
\begin{frame}{practical modeling: ice sheets}

\begin{itemize}
\small
\item the icy region (e.g.~Greenland) is evolving \dots and nearly-fractal
\item limitations in our big ice sheet model:\footnote{$^*$Parallel Ice Sheet Model, \texttt{pism-docs.org}}
  \begin{itemize}
...
\begin{frame}{glacier (and ice sheet) model paradigms}

\begin{itemize}
\item<1> glacier problem \alert<1>{(most numerical modelers)}:
   \begin{quote}
   given geometry of glacier, and stress boundary conditions, determine velocity of ice
   \end{quote}
\item<2> glacier problem \alert<2>{(most glaciologists)}:
   \begin{quote}
...
\item my goal: better ice sheet models \only<2>{\hfill\alert{vs PISM}}
  \begin{itemize}
  \item[$\circ$] effective for long ($\sim 100$ka) and high res ($\sim 1$ km) \only<2>{\hfill\alert{$\sim\checkmark$}}
...
\section{fluid layers (and their climates)}
...
\begin{center}
\includegraphics[width=0.8\textwidth,keepaspectratio=true]{tsunami-sendai}
\end{center}
\end{frame}
...
\begin{center}
\includegraphics[width=0.7\textwidth,keepaspectratio=true]{aral-sea-1964-2014}
\end{center}
\end{frame}
