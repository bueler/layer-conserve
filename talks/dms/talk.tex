...
\item \emph{my goal}: better numerical glacier models
  ...
  \item[$\circ$] without first-order time-splitting errors \only<2>{\hfill\alert{\large\frownie{}}}
  \item[$\circ$] without explicit time-step restrictions \only<2>{\hfill\alert{\large\frownie{}}}
  \end{itemize}
\visible<2>{\item \alert{vs PISM = Parallel Ice Sheet Model, \texttt{pism-docs.org}}}
\end{itemize}
\end{frame}
...
\item setwise statements from the NCP:
    ...
    \item[$\circ$] where $h^\ell = 0$,
        ...
        \vspace{-4mm}
        \begin{itemize}
        \item says ``climate is sufficiently negative to remove old thickness''
        \item i.e.~during time step
        \end{itemize}
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item a nonlinear problem because of constraint
  \begin{itemize}
...
\begin{itemize}
\item this slide: steady case ($\Delta t = \infty$)
    \begin{itemize}
    \item[$\circ$] hard case for numerics
    \end{itemize}
...
\item I have hijacked the word ``climate'' so as to define it to mean whatever I want
     ...
     \visible<2>{\item[$\circ$] want a selfie?}
     \end{itemize}
