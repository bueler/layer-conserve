...
We now require the scheme to satisfy interior conservation.  That is, we require that between any two adjacent \emph{fluid-filled} cells we have flux balance across the edge:
\begin{equation}
...
Many schemes can be given interpretations \eqref{eq:fvthickness}--\eqref{eq:fvmassconserve}, including FV methods for hyperbolic problems \cite{LeVeque2002}, and more-general schemes for diffusive problems \cite{Bueler2016,Morton2018}.  They will differ in how the equations are solved, how the free-boundary conditions are applied, and what are the consequent stability and convergence properties.  Indeed \eqref{eq:fvthickness}--\eqref{eq:fvmassconserve} may not suffice to give a unique scheme even when formulas for the edge fluxes $Q_n^{(j,k)}$ are added, but these axioms suffice to allow the conservation error quantification given next.
...
JouvetBuelerGraeserKornhuber2013}, but we have just applied the FV language of discrete conservation.  These views can be harmonized by observing that a VI is equivalent to a nonlinear complementarity problem (NCP) \cite{FacchineiPang2003,KinderlehrerStampacchia1980}, and both practical solver algorithms and clearer intuition result from this observation.  The dual-mesh schemes described next are both conforming and implementable using finite-dimensional NCP solvers.  The shallow ice sheet solver described in \cite{Bueler2016} serves as an example of the combined FV, FE, and VI/NCP techniques described here.
