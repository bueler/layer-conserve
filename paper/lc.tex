...
Consider a fluid which moves in a thin layer on a fixed substrate.  Fluid mass can be added (accumulation, precipitation) or removed (ablation, evaporation) from the layer by external agents.  Through flow and these addition/removal processes, the spatially-varying thickness of the layer will change in time.  We consider models of such fluid layers in which the fluid geometry is described by a nonnegative thickness function.  In such models the addition/removal processes are combined into a signed source term in a two-spatial-dimension mass conservation equation.  This equation applies in those areas where the thickness is positive, though the addition/removal processes are defined on a larger (supposed fixed) region.  The region of positive thickness, i.e.~the fluid-covered domain, changes in time, and therefore the problem of determining the fluid motion is of free-boundary type.

In physically-complete models the mass conservation equation combines with further momentum and energy conservation equations which together determine the evolution of layer geometry, state, and flow.  Additionally there are, typically, models determining the addition/removal processes, i.e.~the ``climate'' of the fluid layer.  In any case the physics must determine changes in the location of the free/moving boundary at which the thickness goes to zero.  This paper contains a basic analysis of the mathematical well-posedness of such climate-influenced fluid layer models.  We extract the minimal mathematical form which includes flow, climate, and nonnegative-thickness constraint.  In addition we consider certain tradeoffs inherent in numerical solutions of such models.
...
The above geoscientific applications drive the author's interest.  On the other hand the situation is as familiar as the dynamics of rain on a car windshield \cite{Kondic2003}, where precipitation, evaporation, and gravity and/or wind-driven flow all combine to determine the evolution of the liquid water geometry and the wetted and dry domains.  However, the models here do not feature surface tension.
...
\subsection{Advective fluxes (differentiable velocity field)} \label{subsec:advect}  Now consider an advective (hyperbolic) problem for a layer transported by a differentiable velocity field $\bX \in W^{1,\infty}(\Omega;\RR^d)$,
\begin{equation}
  \bQ_n(u,x) = \bX(x) u. \label{eq:advectflux}
\end{equation}
It is easy to check that if $1<p<\infty$ then $\bQ_n$ satisfies the standard flux assumptions on $W^{1,p}(\Omega)$.  Next we let $u,v\in\mathcal{K}$ and use integration-by-parts with $u=v=0$ on $\partial \Omega$ to show
\begin{align}
...
Equation \eqref{eq:advectQnmono} can be exploited in a couple of ways.  If the vector field is divergent $\Div\bX\ge 0$ then \eqref{eq:Qnmonotone} applies with $C=0$ and so $A_n$ is strictly monotone.  Otherwise, \eqref{eq:Qnmonotone} applies with $C = \frac{1}{2}\Delta t_n\,\|(\Div\bX)_{-}\|_{L^\infty(\Omega)}$, so it follows that $A_n$ is monotone if $C \le 1$, and strictly monotone if $C < 1$.  Observe that such a bound on the product of $\Delta t_n$ and a norm of $\bX$ may be regarded as a CFL-type condition \cite{LeVeque2002}, but here the bound measures the convergence of the velocity field.

Consider the operator $A_n$ corresponding to \eqref{eq:advectflux}, i.e.~defined by \eqref{eq:defineAn} as an operator from the convex subset $\mathcal{K} \subset \mathcal{X} = W_0^{1,p}(\Omega)$ to the dual $\mathcal{X}'$.  There is no reason to suppose this operator is coercive.  However, if we add just a little bit of diffusion in the form of a $p$-Laplacian leading-order term with coefficient $\eps>0$ and $p\ge 2$,
\begin{equation}
  \bQ_n(\grad u,u,x) = -\eps |\grad u|^p \grad u + \bX(x) u,   \label{eq:plapadvectflux}
\end{equation}
then the corresponding operator $A_n$ on $W_0^{1,p}(\Omega)$ is coercive.  [FIXME: USE $(\Div \bX)_{-}$]  In fact, combining calculations \eqref{eq:plapQnmono} and \eqref{eq:advectQnmono} gives
\begin{equation}
...
Note that condition \eqref{eq:plapadvectdtcond} is independent of $\eps>0$, suggesting that the pure advection problem ($\eps = 0$) may also be well-behaved if the other hypotheses of the Theorem are satisfied.
