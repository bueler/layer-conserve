...
\begin{abstract}
Time-dependent models of fluid motion in thin layers, subject to signed source terms, such as ice sheets, sea ice, and even inland seas, represent important sub-problems within climate dynamics.  Free boundaries, wherein the thickness and mass flux go to zero at the margin of the fluid, arise in such models.  We address these problems as sequences of discrete-time, continuous-space weak formulations, namely (monotone) variational inequalities or complementarity problems.  After showing these problems are well-posed in specific cases, we consider the limitations to discrete conservation or balance in numerical schemes.  A free boundary in a region of negative source---an ablation-caused margin---is a barrier to exact discrete balance.  However, we offer computable \emph{a posteriori} quantities which allow balance up to rounding error in broad classes of finite volume and element schemes.
\end{abstract}
