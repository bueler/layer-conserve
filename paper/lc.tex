...
Numerical approximations of these time-evolving fluid layer problems necessarily discretize time in some manner, and our analysis is based on this reality.  We semi-discretize the mass conservation equation in time by a one-step method and pose the single-time-step, continuous-space problem in weak variational form.  For the discretized-time problem, sections \ref{sec:timeseries} and \ref{sec:wellposed} address conservation errors and well-posedness, respectively, independently of spatial discretization.  When solving the single-time-step problem numerically, however, a broad selection of spatial-discretization methods are of course available, including finite difference/volume/element methods.  Only in section \ref{sec:spacediscretized}, where we use an unstructured finite volume element \cite{Cai1990} approach to extend the results from the discretized-time case, in our approach is specific to a spatial discretization.
...
In the discretized-time case each time-step requires the solution of a (continuous) free boundary problem in space.  An immediate question about such models is
  ...
  \end{enumerate}
  \end{quote}
The answer to (i) depends on the form of the flux, but we can show (section \ref{sec:wellposed}), by essentially-standard methods applied to the variational inequality form of the weakly-posed problem (section \ref{sec:weakform}), that the answer is often ``yes,'' with sufficient conditions which may include a time-step restriction.
