...
\end{equation}
\cite[Theorem I.5.5]{KinderlehrerStampacchia1980}; see also \cite{FacchineiPang2003}.  (By nonnegativity of the factors the final complementarity condition in \eqref{eq:FEtheNCP} can be regarded either entrywise or as an inner-product.)  NCP \eqref{eq:FEtheNCP} is nonlinear even if the operator $A_n$ is linear, and thus iteration is expected in any numerical solution.  Scalable Newton schemes for NCP problems are described in \cite{BensonMunson2006}; relevant applications appear in \cite{Brinkerhoffetal2017,Bueler2016}.
...
Suppose we recompute the residual using a cell characteristic function $\mathbbm{1}_{\omega_i}$:
\begin{align}
